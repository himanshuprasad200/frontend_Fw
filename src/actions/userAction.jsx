...
// ALL REQUESTS USE PROXY → /api/v1/...
// NO FULL URLS → WORKS IN PRODUCTION
...
export const resetPassword = (resetData) => async (dispatch) => {
  try {
    ...
    const config = { headers: { "Content-Type": "application/json" } };
    const { data } = await axios.put("/api/v1/password/reset", resetData, config);
...
export const getUserDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`/api/v1/admin/users/${id}`, { withCredentials: true });
...
export const deleteUser = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`/api/v1/admin/users/${id}`, { withCredentials: true });
...
export const newReviewForUser = (reviewData) => async (dispatch) => {
  try {
